...
async function retrieveDataFromSource(mesh) {
   await loadMeshFromOBJ(mesh); // Carica il file OBJ
   if (mesh.fileMTL) { // Carica il file MTL se presente
       ...
       if (!mesh.materials || mesh.materials.length === 0) {
           console.warn("Nessun materiale trovato nel file MTL.");
       }
   }
}
...
async function LoadMesh(gl,mesh) {
  ...
  // Unitize(mesh.data); // Normalizza la mesh per mettere le coordinate in un intervallo comune
  
   for (var j = 0; j < mesh.materials.length; j++) { // per iterare su tutti i materiali (mesh.materials) della mesh
      let map = mesh.materials[j].parameter;
      let path = mesh.sourceMesh.substring(0, mesh.sourceMesh.lastIndexOf("/") + 1); // Calcola il percorso per le texture
      ...
      mesh.positions = []; //inizializzare (o resettare) gli array che conterranno le informazioni della geometria
      mesh.normals = [];
      mesh.texcoords = [];
      mesh.texture = tex;
      
       // Riempie le posizioni dei vertici e le coordinate texture
       for (let i = 0; i < nvert; i++) {
           x[i] = mesh.data.vert[i + 1]?.x || 0; //?. verifica se l'oggetto esiste prima di accedere alla proprietà. Se mesh.data.vert[i + 1] è null o undefined, allora || usa come valore di default 0
           y[i] = mesh.data.vert[i + 1]?.y || 0;
           z[i] = mesh.data.vert[i + 1]?.z || 0;
       }

       for (let i = 0; i < ntexcoord - 1; i++) {
           xt[i] = mesh.data.textCoords[i + 1]?.u || 0;
           yt[i] = mesh.data.textCoords[i + 1]?.v || 0;
       }
    ...
    await Promise.resolve(mesh); // Restituisce la mesh una volta completate tutte le operazioni
}
